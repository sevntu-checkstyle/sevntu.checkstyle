...
/**
 * <p>
 * This check prevents the default implementation Serializable interface in inner classes (Serializable interface are
 * default if methods readObject() or writeObject() are not override in class). Check has option, that allow
 * implementation only one method, if it true, but if it false - class must implement both methods. For more information
 * read "Effective Java (2nd edition)" chapter 11, item 74, page 294.
 * </p>
 * 
...
{
	/**
	 * <b> Option, that allow partial implementation of serializable interface. </b>
	 */
	...
	private boolean hasSerialazableMethods(DetailAST aClassNode)
	{
		...
		for(DetailAST methodNode: Utils.iterate(objectBody, TokenTypes.METHOD_DEF))
		{
			if (Utils.isPrivateMethod(methodNode)
	...
	/**
	 * <p>
	 * Return true, if method has correct parameters (ObjectInputStream for readObject() and ObjectOutputStream for
	 * writeObject()).
	 * </p>
	 * 
	 * @param aMethodNode
	 *            - method node.
	 * @param aParameterText
